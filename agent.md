...
- `v2/` is the alternate artist-styled version. Keep it light, gallery-like, and memory-conscious; do not bring back dark full-screen effects unless explicitly requested.
...
- In `v2/`, use `window.getV2ImagePath(src, profile)` from `v2/js/image-paths.js` and keep original files only as fallbacks.
