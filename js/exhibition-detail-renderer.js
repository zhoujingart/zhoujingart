function renderExhibitionDetailPage(exhibition) {
    requestAnimationFrame(() => {
        ...
        const content = `
            ...
            ${exhibition.documents?.length ? `<div class="exhibition-certificates"><h2>${label('exhibitions.documents', '展览文档')}</h2><div class="certificates-grid">${exhibition.documents.map((item, index) => `<div class="certificate-item viewer-trigger" role="button" tabindex="0" data-viewer-type="document" data-viewer-index="${index}" aria-label="${text(item, 'title')}"><img src="${getDetailImagePath(item.image, 'card')}" alt="${text(item, 'title')}" loading="lazy" decoding="async"><div class="certificate-title">${text(item, 'title')}</div></div>`).join('')}</div></div>` : ''}
